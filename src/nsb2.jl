...
function get_coefficients(N::Int64)
	...
	b = [(N-1.0)/(2*N), #b1
        (-2.0+1.0/N)/3, #b2
        (N^2.0 - N - 2.0)/(9.0*(N^2 - N)), #b3
	    (2.0*(2.0 - 3.0*N - 3.0*N2 + 2.0*N3))/(135.0*Nm2*N), #b4
	    (4.0*(22.0 + 13.0*N - 12*N2 - 2.0*N3 + N4))/(405.0*Nm3*N), #b5
	    (4.0*(-40.0 + 58.0*N + 65.0*N2 - 40.0*N3 - 5.0*N4 +	
	      2.0*N5))/(1701.0*Nm4*N), #b6
		#FIXME: the 7th term is wrong as compared to the active implementation
	    (4.0*(-9496.0 - 6912.0*N + 5772.0*N2 + 2251.0*N3 - 
	      1053.0*N4 - 87.0*N5 + 29.0*N6))/(42525.0*Nm5*N), #b7
